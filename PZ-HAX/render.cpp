...
#pragma region utils
	...
	void CalculateTargetOffsets(float target, int screenCenter, float aimSpeed, float lock, float& result)
	{
		if (target != 0)
		{
			if (target > screenCenter)
			{
				result = -(screenCenter - target);
				result /= aimSpeed;
				result += lock;
				if (result + screenCenter > screenCenter * 2) result = 0;
			}
			else if (target < screenCenter)
			{
				result = target - screenCenter;
				result /= aimSpeed;
				result -= lock;
				if (result + screenCenter < 0) result = 0;
			}
		}
	}

	void NormalizeTarget(float& target)
	{
		if (abs(target) < 1)
		{
			if (target > 0)
	...
	{
		int ScreenCenterX = Globals::rWidth / 2;
		int ScreenCenterY = Globals::rHeight / 2;
		float AimSpeed = speed;

		// Calculate the target offsets
		float TargetX = 0, TargetY = 0;
		CalculateTargetOffsets(x, ScreenCenterX, AimSpeed, lock, TargetX);
		CalculateTargetOffsets(y, ScreenCenterY, AimSpeed, lock, TargetY);

		if (smooth)
		{
			TargetX /= 10;
			TargetY /= 10;
			NormalizeTarget(TargetX);
			NormalizeTarget(TargetY);
		}
...
#pragma region menu utils
	...
	void aimbot()
	{
		...
		ImGui::BeginChild(XorStr("#Genral"), ImVec2(270, 175), true);
		{
			...
			ImGui::SliderFloat(XorStr("FOV##1"), &g_Options.AIMBOT.FOV, 1.f, 90.0f, "%1.0");
			ImGui::SliderFloat(XorStr("Smooth"), &g_Options.AIMBOT.aimsmooth, 2.0f, 15.0f, "%0.5");
			ImGui::SliderFloat(XorStr("Lock"), &g_Options.AIMBOT.Lock, 0.0f, 5.0f, "%0.5");
